...
we get $N\geq 4$ and $0.110 \leq \epsilon \leq 0.61$\\
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/pragnidhved/signals-and-systems/blob/main/filter%20design/codes/P1.py
\end{lstlisting}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{fig/P1.png}
    \caption{The Analog Low-Pass Frequency Response for $0.110  \leq \epsilon \leq 0.61$}
...
\end{align}
Substituting respective values in \eqref{eq:Gain_eq_LP} we get $G_{LP}=1.1362$
\begin{align}
     H_{a,LP}(s_L) &= \frac{1.1362}{\brak{s_L-s_1}\brak{s_L-s_2}\brak{s_L-s_3}\brak{s_L-s_4}}\\
     &= \frac{1.1362}{s_{L}^4 + 2.0678s_L^3 + 3.1377s_L^2 + 2.6313s_L + 1.1432}\label{eq:design}
\end{align}
\begin{lstlisting}
...
\begin{align}
    H_{a,BP}\brak{s} &= \frac{302.85s^4}{164499.66s^8 + 3003.4s^7 + 8412.19s^6 + 1126.65s^5 + 1561.67s^4 + 137.14s^3 + 124.65s^2 + 5.41s + 3.61} \label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{align}
    H_{d,BP}(z) &= \frac{302.85z^8 -1211.4z^6 + 1817.1z^4 - 1211.4z^2 + 302.85}{30824.38z^8 - 184685.32z^7 + 528188.0z^6 - 923863.44z^5 + 1075730.52z^4 - 85173.84z^3 + 448996.48z^2 - 144751.6z + 22279.18}
\end{align}
 \begin{lstlisting}
